package org.ewsk.fortunebox.api.objects
...
import org.ewsk.fortunebox.api.enums.BoxConfig
import org.ewsk.fortunebox.api.enums.BoxType
import org.ewsk.fortunebox.api.enums.ItemKey
import org.ewsk.fortunebox.ui.PreviewUI
...
class Box(
    ...
    private val boardCastEnable =
        file.getBoolean(BoxConfig.BoardCastEnable.key, false)
    private val openingBoardCast =
        file.getString(BoxConfig.OpeningBoardCast.key)
    private val completedBoardCast =
        file.getString(BoxConfig.CompletedBoardCast.key)
    private val openingCommandEnable =
        file.getBoolean(BoxConfig.OpeningCommandEnable.key, false)
    private val openingCommands =
        file.getStringList(BoxConfig.OpeningCommands.key)
    private val previewEnable =
        file.getBoolean(BoxConfig.PreviewEnable.key, false)
    private val previewName =
        file.getString(BoxConfig.PreviewName.key) ?: "Chest"
    val requireKeys = file.getInt(BoxConfig.RequireKeys.key, 1)
    val animation =
        file.getString(BoxConfig.Animation.key) ?: "rolling..."
    val hologramEnable =
        file.getBoolean(BoxConfig.HologramEnable.key, false)
    val hologramHeight = file.getDouble(BoxConfig.HologramHeight.key)
    val hologramUpdateInterval =
        file.getInt(BoxConfig.HologramUpdateInterval.key)
    val hologramRange = file.getInt(BoxConfig.HologramRange.key)
    val hologramContent =
        file.getStringList(BoxConfig.HologramContent.key)
    private val prizeMessage =
        file.getStringList(BoxConfig.PrizeMessage.key)
    private val prizeCommands =
        file.getStringList(BoxConfig.PrizeCommands.key)
    ...
    private val key =
        file.getString(BoxConfig.KeyItem.key)?.parseToItemStack()
            ?.let {
                buildItem(it) {
                    file.getString(BoxConfig.KeyName.key)
                        ?.apply { name = this }
                    file.getStringList(BoxConfig.KeyLore.key)
                        .apply { lore += this }
                    if (file.getBoolean(BoxConfig.KeyGlowing.key)) shiny()
                    customModelData =
                        file.getInt(BoxConfig.KeyCustomModelData.key)
                    val namespace =
                        file.getString(BoxConfig.KeyModelNamespace.key)
                    val modelKey =
                        file.getString(BoxConfig.KeyModelKey.key)
    ...
    fun executeCompleted(player: Player, prize: Prize) {
        if (boardCastEnable) {
            if (!completedBoardCast.isNullOrBlank()) {
                val boardCast = completedBoardCast.replace("%prefix%", settings.prefix).replace("%player%", player.name)
                    .replace("%prize%", prize.getPrizeName())
                Bukkit.broadcast(boardCast, "fortunebox.boardcast.common")
            }
        }
